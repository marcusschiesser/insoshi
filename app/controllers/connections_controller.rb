class ConnectionsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if Connection.request(current_person, @contact)
        flash[:notice] = _('Connection request sent!')
  ...
  def update
    ...
    respond_to do |format|
      ...
      when "Accept"
        @connection.accept
        flash[:notice] = _("Accepted connection with %{name}") % {:name => %(<a href="#{person_url(contact)}">#{name}</a>)}
      when "Decline"
        @connection.breakup
        flash[:notice] = _("Declined connection with %{name}") % {:name => name}
      end
      format.html { redirect_to(home_url) }
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      flash[:success] = _("Ended connection with %{name}") % {:name => @connection.contact.name}
